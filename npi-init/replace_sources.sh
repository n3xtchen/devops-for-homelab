...
sed -i 's#\(.*bootstrap.*\) http://deb.debian.org/debian/#\1'" $BOOTSTRAP_URL"'#g' stage0/prerun.sh

# 替换镜像中的中文源
DEBIAN_SECURITY_SOURCE='ftp.cn.debian.org/debian-security'
sed -i 's#deb.debian.org/debian#'"$DEBIAN_PRIMARY_SOURCE"'#g;s#security.debian.org/debian-security#'"$DEBIAN_SECURITY_SOURCE"'#g' stage0/00-configure-apt/files/sources.list

PI_PRIMARY_SOURCE='ftp.cn.debian.org/raspberrypi/debian'
sed -i 's#archive.raspberrypi.org/debian#'"$PI_PRIMARY_SOURCE"'#g' stage0/00-configure-apt/files/raspi.list
